...
/*function buscar_usuario_eliminar()
{
		var usuario_buscar=document.frm_eliminar_usuario.buscar.value;	

		if(usuario_buscar == "" || usuario_buscar ==0 || usuario_buscar==null)
		{  
		alert("No ha escrito el nombre del usuario a buscar");
		document.frm_eliminar_usuario.buscar.focus();		
		}
	  else
		    {
			//alert("entro");
			var str = $("#frm_eliminar_usuario").serialize();
			//alert(str);
			$.ajax({
			...
			success: function(data){
				...
				{	
             	document.frm_eliminar_usuario.txtlogin_usuario.value=data1;
				document.frm_eliminar_usuario.txtpassword_usuario.value=data2;
				document.frm_eliminar_usuario.txtconfirmar_password_usuario.value=data2;
					if(data3==1)
						{
             			document.frm_eliminar_usuario.txtactivo.value="SI";
						}
					else
						{
						document.frm_eliminar_usuario.txtactivo.value="NO";	
						}
             	document.frm_eliminar_usuario.txtcreacion.value=data5;
				document.frm_eliminar_usuario.txtvencimiento.value=data6;
				document.frm_eliminar_usuario.txtid_usuario_eliminar.value=data7;
				}
				//document.frm_nuevo_usuario.txtcadena.value=data;
			}
		});
...
{
		var id_usuario_eliminar=document.frm_eliminar_usuario.txtid_usuario_eliminar.value;	

		if(id_usuario_eliminar == "" || id_usuario_eliminar ==0 || id_usuario_eliminar==null)
		{  
		alert("No ha escrito el nombre de usuario a eliminar");
		document.frm_eliminar_usuario.buscar.focus();		
		}
	  else
		    {
...
function buscar_usuario_modificar()
{
		//alert("entro");
		var usuario_buscar=document.frm_modificar_usuario.buscar.value;	

		if(usuario_buscar == "" || usuario_buscar ==0 || usuario_buscar==null)
		{  
		alert("No ha escrito el nombre del usuario a buscar");
		document.frm_modificar_usuario.buscar.focus();		
		}
	  else
		    {
			//alert("entro");
			...
			success: function(data){
				...
				{	
             	document.frm_modificar_usuario.txtlogin_usuario.value=data1;
				document.frm_modificar_usuario.txtpassword_usuario.value=data2;
				document.frm_modificar_usuario.txtconfirmar_password_usuario.value=data2;
				document.frm_modificar_usuario.cmbactivo_usuario.value=data3;
             	document.frm_modificar_usuario.datepicker1.value=data5;
				document.frm_modificar_usuario.datepicker2.value=data6;
				document.frm_modificar_usuario.txtid_usuario_modificar.value=data7;
				}
				//document.frm_nuevo_usuario.txtcadena.value=data;
			}
		});
	...
	if(nombre_agente=="" || nombre_agente==0 || nombre_agente==null)
	...
	else if(correo_agente=="" || correo_agente==0 || correo_agente==null)
		{
			...
			success: function(data){
				...
				if(data=="E")
				{	
              	alert("Error en el sistema, no se guardo el proveedor");
				}
				else(data=="Y")
				{	
...
function modificar_producto_inventaro(id_producto_inventario)
{
	...
	//location.href = location.href + '?var1=32&var2=10';
	document.location.href ="../modificar_producto_inventario.php"+'?idproductoinventario='+id_producto_inventario;		
}
...
function buscar_producto_venta_cb()
{
	...
	var cb_producto=document.frm_nueva_venta.txtcodigo_barras.value;	
...
function guardar_venta()
{
		...
		var cambio_venta=document.frm_nueva_venta.cambio_venta.value;
		var totalpagar=document.frm_nueva_venta.totalpagar.value;		
  	...
  	{	
		...
		if(totalpagar==0)
		...
		else
			{
				...
				document.frm_nueva_venta.cambio_venta.value=cambio_venta;
				document.frm_nueva_venta.txtcambio_venta.value=cambio_venta;
...
function corte_caja()
{	
window.open('../reportes/tcpdf/examples/rptcorte_caja.php?fecha_inicial=' + document.getElementById('datepicker1').value + "&fecha_final=" + document.getElementById('datepicker2').value, 'ventana1' ,'left=10,top=10,width=900,height=600,scrollbars=yes'); 			
}
  ...
  if (tecla==13)
  	{	
		...
		if(total_venta==0)
			...
			{
				var totalpagar=total_venta-descuento;
...
/*function guardar_producto_venta_buscado(e,cantidad,id_producto) {
  ...
  if (tecla==13)
  	{
		document.frm_lista_productos_buscar.txtid_producto.value=id_producto;
		document.frm_lista_productos_buscar.txtcantidad_producto.value=cantidad;
		//alert ('Has pulsado enter');
		//alert("entro buscar producto venta");	
			//alert("entro");
